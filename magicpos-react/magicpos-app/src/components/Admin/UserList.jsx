...
const UserList = forwardRef((props, ref) => {
  const { users, pagination, onSearch, onEdit, onRegister, onDelete, onPageChange, onView, selectedUserNos, setSelectedUserNos } = props;
        ...
        <tbody>
          {(users || []).map((user, idx) => (
            <tr key={user.no}
              ...
              <td><input 
                    type="checkbox"
                    checked={selectedUserNos.includes(user.no)}
                    onChange={e => handleCheckboxChange(user.no, e.target.checked)}
